...
// crdSchemaChanges contains the detected schema changes between CRD API versions.
// This data is generated by the crddiff tool (cmd/schemadiff) during code generation
// and is used to automatically register conversion functions at runtime.
//
// Generation: This file is automatically generated by the go:generate directive in
// generate/generate.go. It should be regenerated whenever CRDs change.
//
// Format: JSON mapping "{group}/{kind}" to change reports containing field additions,
// deletions, and type changes between versions (e.g., v1beta1 → v1beta2).
...
func GetProvider(ctx context.Context, sdkProvider *schema.Provider, generationProvider bool) (*ujconfig.Provider, error) {
	...
	if err := ujconfig.ExcludeTypeChangesFromIdentity(pc, crdSchemaChanges); err != nil {
		return nil, errors.Wrap(err, "cannot exclude the CRD field type changes from the provider")
	}
	bumpVersionsWithEmbeddedLists(pc)
	if !generationProvider {
		if err := ujconfig.RegisterAutoConversions(pc, crdSchemaChanges); err != nil {
			return nil, errors.Wrap(err, "cannot inject the CRD schema change conversion functions")
		}
	}
	return pc, nil
}
